...
export default class MyTelegramOrg {
    ...
    loginWithCode = async (code) => {
        ...
        if (result === 'Invalid confirmation code!') {
            ...
            const rawCookiesHeaders = responseRaw.headers.get('Set-Cookie');
    ...
    obtainTokens = async () => {
        if (!this.sessionToken)
            throw new Error('Session token not found. You must be logged in to obtain tokens.');
        ...
        if (!appIDEl)
            throw new Error('Unable to find app id element');
        const appHashEl = root.querySelector('[for=app_hash]+div > span');
        if (!appHashEl)
            throw new Error('Unable to find app hash element');
